...
import type { LocalNode, LocalEdge, GitHubInfo } from '@monoid/analyzer-core';
...
async function claimJob(): Promise<IngestJob | null> {
 ...
 */
async function claimJobFallback(): Promise<IngestJob | null> {
...
async function processJob(job: IngestJob): Promise<void> {
  ...
  try {
    ...
    const result = await analyzeDirectory(tmpDir, githubInfo, {
...
async function cloneRepo(job: IngestJob, targetDir: string): Promise<void> {
  let repoUrl: string;
  if (job.github_token) {
...
async function createVersion(repoId: string, commitSha: string, branch: string): Promise<RepoVersion> {
...
async function saveNodes(versionId: string, nodes: LocalNode[]): Promise<Map<string, string>> {
...
async function saveEdges(
  versionId: string,
  edges: LocalEdge[],
  stableIdToId: Map<string, string>
): Promise<void> {
...
async function updateVersionCounts(versionId: string, nodeCount: number, edgeCount: number): Promise<void> {
...
async function markJobSucceeded(jobId: string, versionId: string): Promise<void> {
  const { error } = await supabase
    .from('ingest_jobs')
    .update({
      status: 'succeeded',
...
async function markJobFailed(job: IngestJob, errorMessage: string): Promise<void> {
...
async function ensureClaimRpc(): Promise<void> {
