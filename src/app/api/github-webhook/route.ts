...
import { analyzeFeasibility, analyzeAndVisualize } from "@/lib/github-agent/agent";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (shouldProcessCommentEvent(event, data.action)) {
      ...
      const isVisualize = isVisualizationRequest(monoidCommand);

      // Run analysis and wait for completion
      // GitHub webhooks have a 10s timeout, but we extend via maxDuration
      await runAnalysisInBackground(analysisParams, requestId, isVisualize);

      return NextResponse.json({
        message: isVisualize ? "Visualization complete" : "Analysis complete",
        requestId,
        issueNumber: issue.number,
        trigger: "comment",
        command: monoidCommand,
        type: isVisualize ? "visualization" : "feasibility",
      });
    }
...
/**
 * Run the analysis (feasibility or visualization) and post results
 */
async function runAnalysisInBackground(
  ...
  const analysisType = isVisualization ? "visualization" : "feasibility analysis";
  console.log(`[${requestId}] Starting ${analysisType}...`);
  ...
  try {
    ...
    if (isVisualization) {
      ...
      const visualization = await analyzeAndVisualize(params);
      console.log(`[${requestId}] Visualization complete. Affected nodes: ${visualization.affectedNodes.length}`);
      comment = visualization.comment;
    } else {
      // Run feasibility analysis
      const analysis = await analyzeFeasibility(params);
      console.log(`[${requestId}] Analysis complete. Score: ${analysis.feasibilityScore}/10, Complexity: ${analysis.estimatedComplexity}`);
      comment = analysis.comment;

      // Add feasibility label for feasibility analysis only
    ...
    console.error(`[${requestId}] Error during ${analysisType}:`, error);
    
    // Try to post an error comment
    if (params.installationId) {
      try {
        const errorType = isVisualization ? "Visualization" : "Feasibility Analysis";
        await postIssueComment({
          owner: params.repoOwner,
          repo: params.repoName,
          issueNumber: params.issueNumber,
          body: `## ${errorType} Failed\n\nAn error occurred while processing this issue. Please try again later or contact support.\n\n---\n<sub>Generated by [Monoid](https://monoid.so)</sub>`,
          installationId: params.installationId,
        });
      } catch (commentError) {
